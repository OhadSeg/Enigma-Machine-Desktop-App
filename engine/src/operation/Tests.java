...
public class Tests {
    ...
    protected void runMachineDetailsTests(Enigma mEnigma, TestedMachineDetailsDTO machineDetailsFromDesktopUI) throws MachineDetailsFromUserException {
        StringBuilder userInputExceptionMessage = new StringBuilder();

        if (!checkIfAllchosenRotorsAreDifferentFromEachOther(machineDetailsFromDesktopUI.getChosenValidInUseRotors())) {
        ...
        if (!checkIfAllRotorsAreSelected(machineDetailsFromDesktopUI.getChosenValidInUseRotors(), mEnigma.getAmountOfRotorsInUse())) {
            userInputExceptionMessage.append("USER INPUT ERROR 202 : There are empty fields in the selection of rotors.");
            userInputExceptionMessage.append(System.lineSeparator());
        }
        if (!checkIfAllInitialPosAreSelected(machineDetailsFromDesktopUI.getChosenValidStartPosRotors())) {
        ...
        if (!checkIfReflectorIsSelected(machineDetailsFromDesktopUI.getChosenValidReflector())) {
            userInputExceptionMessage.append("USER INPUT ERROR 204 : There are empty field in the selection of the reflector.");
            userInputExceptionMessage.append(System.lineSeparator());
        }
        if (!checkIfPlugBoardIsValid(machineDetailsFromDesktopUI.getChosenValidPlugs())) {
...
//        }
    ...
    protected void runTextToCodeTests(String textToCode, Enigma mEnigma) throws UserInputException {
        StringBuilder userInputExceptionMessage = new StringBuilder();
    ...
    protected void runBruteForceDetailsFromUserTest(BruteForceDetailsDTO bfDetails)throws UserInputException {
        StringBuilder userInputExceptionMessage = new StringBuilder();
